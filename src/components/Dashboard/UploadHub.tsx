    ...
    <div className="max-w-4xl mx-auto py-10 px-4 sm:px-6">
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-navy-900 tracking-tight">
          Secure Document Ingestion Hub
        </h2>
        <p className="text-sm text-slate-500 max-w-xl mx-auto mt-2">
          Drop sensitive documents below for local PII analysis. Zero server disk retention guarantee.
        </p>
      </div>
      ...
      {!processing ? (
        ...
        >
          ...
          <div className="flex items-center justify-center space-x-3 text-xs text-slate-400">
            <div className="flex items-center space-x-1">
              ...
              <span>Vector Text PDFs</span>
            </div>
            <span>•</span>
            <div className="flex items-center space-x-1">
              <ImageIcon className="w-4 h-4 text-emerald-500" />
              <span>Scanned Images & OCR</span>
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-subtle text-center">
          ...
          <p className="text-xs text-slate-500 mt-1 mb-6">Running local zero-retention analysis engine...</p>
          ...
          <div className="space-y-4 max-w-md mx-auto text-left">
            <div className="flex items-center space-x-3">
              ...
              <span className={`text-xs font-semibold ${currentStep >= 1 ? 'text-navy-900' : 'text-slate-400'}`}>
                1. Parsing Structure & OCR Extraction
              </span>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className={`text-xs font-semibold ${currentStep >= 2 ? 'text-navy-900' : 'text-slate-400'}`}>
                2. Running Hybrid NER & Rule Checksums (Aadhaar/PAN/SSN/Cards)
              </span>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className={`text-xs font-semibold ${currentStep >= 3 ? 'text-navy-900' : 'text-slate-400'}`}>
                3. Inspecting Metadata & Visual Artifact Contours
              </span>
            </div>
          </div>
      ...
      {!processing && (
        <div className="mt-6 text-center">
          <button
            onClick={() => {
              const sampleFile = new File(['Sample Document Content'], 'Employee_Onboarding_Form.pdf', { type: 'application/pdf' });
          ...
          >
            <span>Or click here to test with a pre-loaded sample document</span>
