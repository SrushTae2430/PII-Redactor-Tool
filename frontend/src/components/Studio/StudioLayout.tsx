...
import type { ProcessedDocument, PIIEntity, ProcessingOptions, RiskMetrics, AuditCertificate, RedactionAction } from '../../types';
...
export const StudioLayout: React.FC<StudioLayoutProps> = ({
  ...
  const handleSetEntityAction = (id: string, action: RedactionAction) => {
    setEntities((prev) =>
      prev.map((e) => (e.id === id ? { ...e, action, active: true } : e))
    );
  };
  ...
  const handleSetCategoryAction = (category: 'DIRECT' | 'INDIRECT' | 'VISUAL', action: RedactionAction) => {
    setEntities((prev) =>
      prev.map((e) => (e.category === category ? { ...e, action } : e))
    );
  };
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        <InspectionPanel
          ...
          onSetEntityAction={handleSetEntityAction}
          onToggleCategory={handleToggleCategory}
          onSetCategoryAction={handleSetCategoryAction}
          options={options}
