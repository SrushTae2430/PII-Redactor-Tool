...
def calculate_privacy_risk_scores(detected_entities: List[Dict[str, Any]], redacted_count: int) -> Dict[str, Any]:
    """
    Computes Initial Privacy Risk Score and Post-Sanitization Risk Score (0-100 scale).
    """
    initial_score = 0
    direct_count = 0
    indirect_count = 0
    ...
    for ent in detected_entities:
        ...
        if category == "DIRECT":
            direct_count += 1
            if ent_type in ["AADHAAR", "SSN", "CREDIT_CARD", "PAN"]:
                initial_score += 25
            else:
                initial_score += 15
    ...
    # Cap initial score at 100
    initial_score = min(100, max(12, initial_score))

    # Calculate post redaction risk
    ...
    return {
        ...
        "narrative_summary": f"Scrubbed {redacted_count} sensitive data targets ({direct_count} direct, {visual_count} visual artifacts). Physical layout preserved with 0 server storage."
    }
...
def generate_audit_certificate(filename: str, risk_metrics: Dict[str, Any], metadata_purged: bool, canary_id: str = None) -> Dict[str, Any]:
    return {
        ...
        "local_engine_signature": "PII Shield v1.0.0 Zero-Retention Local Pipeline"
    }
